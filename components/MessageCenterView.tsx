...
const MessageCenterView: React.FC<MessageCenterViewProps> = ({ onBack, currentUser, onOpenCompleteSignUp }) => {
    ...
    return (
        <div className="w-full max-w-sm mx-auto bg-gray-100 h-screen flex flex-col font-sans">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
                {/* Persistent Unread Sign Up Notification if Profile Incomplete */}
                {currentUser && !currentUser.isProfileCompleted && (
                    <div className="p-4 bg-gradient-to-r from-amber-500 to-yellow-500 text-black rounded-2xl shadow-md border border-amber-400 space-y-2 relative overflow-hidden">
                        <div className="flex items-start justify-between">
                            ...
                            <span className="bg-red-600 text-white text-[8px] font-black uppercase px-2 py-0.5 rounded-full flex-shrink-0 animate-pulse">
                                UNREAD
                            </span>
                        </div>
                        {onOpenCompleteSignUp && (
                            <button
                                ...
                                className="w-full bg-black text-amber-400 font-black py-2.5 rounded-xl text-xs uppercase tracking-wider hover:bg-gray-900 transition-all shadow-sm active:scale-95"
                            >
