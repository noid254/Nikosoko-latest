    ...
    return (
        <div className="bg-white rounded-lg shadow-sm p-4 max-w-md mx-auto">
           ...
           <div className="space-y-3 mb-4">
               ...
               <select value={broadcastLocation} onChange={e => setBroadcastLocation(e.target.value)} className="w-full p-2 border rounded-md bg-white text-gray-900">
                   {uniqueLocations.map(l => <option key={l} value={l}>{l}</option>)}
               </select>
               <select value={broadcastCategory} onChange={e => setBroadcastCategory(e.target.value)} className="w-full p-2 border rounded-md bg-white text-gray-900">
                   {uniqueCategoriesForBroadcast.map(c => <option key={c} value={c}>{c}</option>)}
               </select>
               <select value={broadcastRating} onChange={e => setBroadcastRating(e.target.value)} className="w-full p-2 border rounded-md bg-white text-gray-900">
                   {ratingOptions.map(r => <option key={r} value={r}>{r}</option>)}
               </select>
           </div>

           <textarea rows={5} value={broadcastMessage} onChange={(e) => setBroadcastMessage(e.target.value)} placeholder="Type your notification message to all users here..." className="w-full p-2 border border-gray-300 rounded-md focus:ring-brand-primary focus:border-brand-primary text-gray-900 placeholder-gray-400"/>
