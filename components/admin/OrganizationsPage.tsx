...
const LeaderInput: React.FC<{
    ...
    return (
        <div>
            ...
            <div className={`flex items-center mt-1 border ${borderClass} rounded-md shadow-sm focus-within:ring-1 focus-within:ring-brand-primary bg-white`}>
                ...
                <input
                    ...
                    className="block w-full pl-1 pr-3 py-2 border-0 rounded-md focus:outline-none focus:ring-0 sm:text-sm text-gray-900 placeholder-gray-400"
                    required
                />
            </div>
...
const OrganizationsPage: React.FC<OrganizationsPageProps> = ({ providers, onCreate, onApproveRequest, onRejectRequest }) => {
    ...
    const inputClass = "w-full p-2 border rounded bg-white text-gray-900 placeholder-gray-400";
        ...
        <div className="bg-white rounded-lg shadow-sm p-4">
            ...
            {isCreating && (
                 <form onSubmit={handleSubmit} className="p-4 border rounded-lg bg-gray-50 space-y-3 mb-6">
                    <h3 className="font-semibold text-lg text-gray-800">Create New Organization</h3>
                    <input value={formState.name} onChange={e => setFormState(p=>({...p, name: e.target.value}))} placeholder="Organization Name" className={inputClass} required />
                    <input value={formState.referralCode} onChange={e => setFormState(p=>({...p, referralCode: e.target.value.toUpperCase()}))} placeholder="Referral Code (e.g. UON2024)" className={`${inputClass} font-mono uppercase`} required />
                    <input value={formState.service} onChange={e => setFormState(p=>({...p, service: e.target.value}))} placeholder="Service / Industry" className={inputClass} required />
                    <input value={formState.location} onChange={e => setFormState(p=>({...p, location: e.target.value}))} placeholder="Location" className={inputClass} required />
                    <textarea value={formState.about} onChange={e => setFormState(p=>({...p, about: e.target.value}))} placeholder="About the organization" className={inputClass} required />
                     <h4 className="font-semibold pt-2 text-gray-700">Leadership Contacts (must be registered users)</h4>
                    <LeaderInput 
