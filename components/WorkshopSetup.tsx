...
const InputField: React.FC<{ label: string, value: string, onChange: (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => void, placeholder: string, required?: boolean, isTextarea?: boolean }> = ({ label, value, onChange, placeholder, required = false, isTextarea = false }) => (
    <div>
        ...
        {isTextarea ? (
            <textarea value={value} onChange={onChange} placeholder={placeholder} required={required} rows={4} className="w-full p-3 border-2 border-gray-200 rounded-lg bg-gray-50 text-gray-900 focus:bg-white focus:ring-2 focus:ring-brand-gold focus:outline-none transition placeholder-gray-400" />
        ) : (
            <input value={value} onChange={onChange} placeholder={placeholder} required={required} className="w-full p-3 border-2 border-gray-200 rounded-lg bg-gray-50 text-gray-900 focus:bg-white focus:ring-2 focus:ring-brand-gold focus:outline-none transition placeholder-gray-400" />
        )}
    </div>
);
                ...
                return (
                    <div className="space-y-6">
                        <div className="space-y-4 text-sm">
                            ...
                            <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg"><span>Business Name</span><span className="font-semibold text-right text-gray-900">{businessName}</span></div>
                            <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg"><span>Address</span><span className="font-semibold text-right whitespace-pre-line text-gray-900">{address}</span></div>
                            <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg"><span>Contact</span><span className="font-semibold text-right text-gray-900">{contactInfo}</span></div>
                            <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg"><span>Payment Info</span><span className="font-semibold text-right whitespace-pre-line text-gray-900">{paymentDetails}</span></div>
                        </div>
                         <div className="text-xs p-3 border rounded-lg h-24 overflow-y-auto bg-gray-50 text-gray-600">
                        ...
                        <label className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg cursor-pointer">
                            ...
                            <span className="font-semibold text-sm text-gray-800">I agree to the terms and conditions</span>
                        </label>
                        <div className="flex gap-3">
                             <button type="button" onClick={handleBack} className="flex-1 bg-gray-200 text-gray-800 font-bold py-4 rounded-xl">Back</button>
